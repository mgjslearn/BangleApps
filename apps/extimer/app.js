...
function showCounter(withHint) {
  ...
  g.drawString("HW Timer! ",80,55);
  g.setFont("Vector",60); // vector font, 60px  
  g.drawString(timeFormated(counter),83,100);
  if (withHint) {
    g.setFont("Vector",15); // vector font, 80px
    g.drawString("Tap to finish HWtime!",90,150);
  }
}
...
function doneWHw(){
    E.showMessage("YAY! You're finally done. Now you can play!");
}
...
// show hwtime text while in start state (see initDragEvents())
function showHWTimer() {
  if (state == states.start) {
    state = states.hwtime;
	g.setBgColor(g.theme.bg);
	g.setColor(g.theme.fg);
    E.showMessage("Do you have HW left? (NO: press BTN; YES: Tap Screen", "HW Timer");
  }
  // return to start
  else if (state == states.hwtime) {
    initTimer();
  }
}

require("Storage").write("extimer.info",{
  "id":"extimer",
  "name":"ExTimer",
  "src":"extimer.app.js",
  "icon":"extimer.png"
...
initDragEvents();
// Show hwtime test in start state
Bangle.on('touch', function(button, xy) { showHWTimer(); });
